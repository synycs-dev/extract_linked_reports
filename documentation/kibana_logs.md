...
If address `http://cluster_ip:30081` is not available, ensure that you used the `--with-elk` flag on cluster installation step ([GitHub - hystax/synops](https://github.com/hystax/synops/tree/integration?tab=readme-ov-file#cluster-update))  and `elk-0` pod in the running state.
