...
> - make sure you backed up data folder/volume ``` /SynOps ```
> - make sure you backed up user overlays ``` ~/SynOps/SynOps-deploy/overlay/ ```

In this example instruction data directory is placed in ```/SynOps```, sources placed in ```~/SynOps```

#### 1. navigate to SynOps-deploy repo:
...
``
~/SynOps/SynOps-deploy$ source .venv/bin/activate
``

#### 3. delete deployment

```
~/SynOps/SynOps-deploy$ ./runkube.py -d  -- <release_name> local
```

if you're forgotten release name, you can use this command:
```
$ helm list | grep SynOps
```
The output should be like:
```
$ helm list | grep SynOps
NAME         	REVISION	UPDATED                 	STATUS  	CHART               	APP VERSION	NAMESPACE
os-1      	1       	Thu Dec 26 12:32:35 2024	DEPLOYED	SynOps-0.1.0      	           	default
```

according to the output, my release is: __os-1__ and the command will be:

```
~/SynOps/SynOps-deploy$ ./runkube.py -d  -- os-1 local
```
```
12:29:10.285: Deleting SynOps cluster os-1 on k8s 172.24.1.14
...
```
~/SynOps/SynOps-deploy$ deactivate
~/SynOps/SynOps-deploy$ rm -r .venv/
```
...
#### 11. pull new version
```
~/SynOps$ git fetch && git pull
```

#### 12. create new venv
```
~/SynOps$ cd SynOps-deploy/
```

```
~/SynOps/SynOps-deploy$ virtualenv -p python3 .venv
```
```
~/SynOps/SynOps-deploy$ source .venv/bin/activate
```
...
```
~/SynOps/SynOps-deploy$ pip install -r requirements.txt
```
...
```
~/SynOps/SynOps-deploy$  ansible-playbook -e "ansible_ssh_user=<user>" -k -K -i "<ip>," ansible/k8s-master.yaml
```
